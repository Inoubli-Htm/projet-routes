...
import FilmsListes from './components/FilmsListes';
...
const getRate=(rate)=>{
  ...
  return (
    <BrowserRouter>
    <div className="App">
        <NavBar getText={getText} getRate={getRate} />
      
        <Routes>
      
          <Route path='/' element={<>
            <AddFilms addFilms={addFilms} />
            <FilmsListes films={films} rate={rate} text={text} />
          </>} />
          <Route path='/Trailer/:trailerId' element={<Trailer films={films} />} />
        </Routes>
    </div>
    </BrowserRouter>
  );
